...
## 19-A � Building Blocks: AggregateRoot, ValueObject, Money
...
public abstract class AggregateRoot
{
    ...
    protected void RaiseDomainEvent(INotification @event) => _domainEvents.Add(@event);
...
// ?? Money ?? value object as record struct (structural equality, no ORM issues)
...
// ?? BankAccountAggregate ?? owns and enforces invariants
public class BankAccountAggregate : AggregateRoot
{
    private BankAccountAggregate() { }
    public static BankAccountAggregate Open(string accountNumber, string ownerName, Money initialBalance)
    {
        // validate ? mutate ? raise event
        var agg = new BankAccountAggregate { AccountNumber = accountNumber, ... };
        agg.RaiseDomainEvent(new AccountCreatedDomainEvent(...));
        return agg;
    }
    public void Deposit(Money amount)  { /* validate ? Balance = Balance.Add(amount) */ }
    public void Withdraw(Money amount) { /* validate ? Balance = Balance.Subtract(amount) */ }
}
```

**Java parallel:**  
`Axon @Aggregate` + `@CommandHandler` ? `BankAccountAggregate.Open/Deposit/Withdraw`.  
`AbstractAggregateRoot<T>` domain events ? `AggregateRoot.RaiseDomainEvent`.  
Vavr immutable value types ? `record struct`.

## 19-B � Repository Abstraction + Anti-Corruption Layer
...
// Infrastructure layer: maps domain ? EF entity, dispatches domain events post-save
public class EfAggregateRepository(BankingDbContext db, IPublisher publisher) : IAggregateRepository
{
    public async Task AddAsync(BankAccountAggregate agg, CancellationToken ct = default)
    {
        db.BankAccounts.Add(MapToEntity(agg));
        await db.SaveChangesAsync(ct);
        agg.Id = entity.Id;                     // copy DB-assigned id back
        await DispatchDomainEvents(agg);         // dispatch after commit
    }
    public async Task SaveAsync(BankAccountAggregate agg, CancellationToken ct = default)
    {
        var entity = await db.BankAccounts.FindAsync([agg.Id], ct);
        entity!.Balance = agg.Balance.Amount;   // update tracked entity
        await db.SaveChangesAsync(ct);
        await DispatchDomainEvents(agg);
    }
}

// Reconstruct aggregate from persistence � no invariant enforcement needed
...
**Java parallel:**  
Spring Data repository interface ? `IAggregateRepository`.  
...
## 19-C � Bounded Contexts, IUnitOfWork + Domain Events, Optimistic Concurrency

| Concept | C# | Java |
|---|---|---|
| Domain event dispatch post-commit | `IPublisher.Publish(events)` after `SaveChangesAsync` | `@AfterDomainEventPublication` |
| Optimistic concurrency | `[Timestamp]` + `DbUpdateConcurrencyException` | `@Version` + `OptimisticLockException` |
| Bounded context boundary | Separate namespace / assembly | Separate Spring module / package |

## Tests
```bash
dotnet test --filter "FullyQualifiedName~DddBuildingBlockTests"  # 9 tests
dotnet test --filter "FullyQualifiedName~DddRepositoryTests"     # 4 tests
dotnet test --filter "FullyQualifiedName~DddAdvancedTests"       # 4 tests
```
...
# Lesson 21-C â€” OpenAPI + Scalar UI
...
| Topic | .NET | Java |
|---|---|---|
| OpenAPI document generation | `Microsoft.AspNetCore.OpenApi` â†’ `/openapi/v1.json` | springdoc-openapi â†’ `/v3/api-docs` |
| Document enrichment | `AddDocumentTransformer` in `AddOpenApi()` | `@OpenAPIDefinition(info = @Info(...))` |
| Interactive UI | `Scalar.AspNetCore` â†’ `/scalar/v1` | Springfox Swagger UI / springdoc-openapi |
...
builder.Services.AddOpenApi("v1", options =>
{
	options.AddDocumentTransformer((doc, ctx, ct) =>
	{
		...
		doc.Info.Version     = "v1";
		doc.Info.Description = "ASP.NET Core 10 Banking Curriculum â€” Lesson 21-C";
		return Task.CompletedTask;
	});
});

// Program.cs â€” middleware pipeline
if (app.Environment.IsDevelopment())
{
	...
	{
		options.Title             = "Banking API â€” Scalar UI";
...
2. Add `[EndpointSummary]` and `[EndpointDescription]` attributes to Minimal API endpoints â€” confirm they appear in `/openapi/v1.json`.
...
4. Swap `ScalarTheme.Purple` to `ScalarTheme.DeepSpace` â€” observe the live UI change without a redeploy.
