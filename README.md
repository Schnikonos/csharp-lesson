# Lesson 06-C � Advanced Filters (IResourceFilter, IResultFilter, IEndpointFilter)
...
## Rate Limiting (06-C Extended)

ASP.NET Core 7+ ships rate limiting middleware in-box â€” no extra NuGet package required.

### Three algorithms compared

| Algorithm | Policy name | How it works | Java parallel |
|---|---|---|---|
| **Fixed window** | `fixed` | N requests per T-second window; hard reset at boundary | `Bucket4j Bandwidth.simple` |
| **Sliding window** | `sliding` | Same limit, window slides by sub-windows â€” no boundary burst | Resilience4j sliding window |
| **Token bucket** | `token` | Bucket of tokens; refilled at a steady rate; burst-friendly | Guava RateLimiter / Resilience4j |

### Key files

| File | Role |
|---|---|
| `Controllers/RateLimitController.cs` | Three endpoints, one per algorithm |
| `Program.cs` â†’ `AddRateLimiter` | Policy registration + `UseRateLimiter()` middleware |
| `Lesson.Tests/RateLimitTests.cs` | 4 integration tests covering all three algorithms |

### Rate-limit tests

| Test | What it verifies |
|---|---|
| `FixedWindow_ExceedsLimit_Returns429` | 11th request crosses fixed-window limit |
| `SlidingWindow_ExceedsLimit_Returns429` | 11th request crosses sliding-window limit |
| `TokenBucket_ExceedsInitialGrant_Returns429` | 21st request exhausts token bucket |
| `UnderLimit_AllRequestsSucceed` | Requests within limit always get 200 |

### Additional exercises

5. Add a **concurrency limiter** policy (`AddConcurrencyLimiter`) that caps simultaneous in-flight requests to 5.
6. Implement a **per-user** partition key using `partitionedLimiter`: key off `HttpContext.User.Identity.Name`.
