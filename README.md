...
## Testcontainers (12-C Extended)

Testcontainers spins up a **real PostgreSQL Docker container** for the test run and tears it down automatically â€” no Docker Compose, no shared state.

### Why Testcontainers?

| | SQLite in-memory | Testcontainers PostgreSQL |
|---|---|---|
| Speed | Fastest | Slower (Docker pull + start) |
| SQL fidelity | Limited | Full PostgreSQL semantics |
| JSON / array ops | Not supported | Native |
| Constraint behaviour | Close but not identical | Exact |
| CI requirement | None | Docker on build agent |

### Pattern

```csharp
// IAsyncLifetime â€” start/stop container once per class
...
public async Task InitializeAsync()
{
	...
	// replace DbContextOptions with Npgsql connection string
	// run db.Database.MigrateAsync()
}
...
```bash
# Run all tests including Docker tests (Docker must be running)
dotnet test

# Skip Docker tests in environments without Docker
...
| File | Role |
|---|---|
| `Lesson.Tests/TestcontainersIntegrationTests.cs` | 3 real-PostgreSQL integration tests |
| `Lesson.Tests/Lesson.Tests.csproj` | `Testcontainers.PostgreSql` + `Npgsql.EntityFrameworkCore.PostgreSQL` |

### Testcontainers tests

| Test | What it verifies |
|---|---|
| `CreateAccount_ThenGetById_ReturnsAccount` | Full write/read round-trip on real PostgreSQL |
| `CreateAccount_DuplicateNumber_Returns409` | Unique constraint enforced by real DB |
| `GetAccounts_ReturnsPersistedRows` | EF Core seed via scope + HTTP list |

### Additional exercises

4. Add a Testcontainers test for a bulk INSERT of 1 000 accounts and assert `GET /accounts` returns all 1 000.
5. Replace `postgres:16-alpine` with `postgres:15-alpine` â€” confirm the tests still pass (image version pin strategy).
6. Add a second container (`RedisContainer`) and use it to test output caching backed by real Redis.
